...
```
  ...
  This repository is currently a local workspace, not a published global npm CLI. The wrapper above is the normal source-checkout install path for agent clients that can execute shell commands. A portable project-local launcher is also written during `nullius init`, so already-initialized research folders can continue with `./.nullius/bin/nullius status --json` even when the global wrapper is absent. On native Windows, use the generated companion `.\.nullius\bin\nullius.cmd status --json`.
...
```
  ...
  Once initialized, reconnect is local-first: `.nullius/HARNESS`, the project-local launcher (`.nullius/bin/nullius`, plus `.nullius/bin/nullius.cmd` on Windows), `AGENTS.md`, `research_plan.md`, `research_contract.md`, and `artifacts/runs/<run_id>/` are enough for an agent to recover the project state after a closed session or a network outage. Network access is only needed for tasks that actually fetch external sources.
