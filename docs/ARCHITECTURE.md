...
- `nullius init` writes `.nullius/HARNESS` as the machine-readable project handshake. It also writes a portable `.nullius/bin/nullius` recovery launcher, with `.nullius/bin/nullius.cmd` as the native Windows companion. Agents should treat the sentinel's presence as a mandatory `nullius status --json` receipt trigger before new work, milestone execution, closeout, or handoff. `nullius init --refresh` re-applies the managed scaffold doc (`AGENTS.md`) into an already-initialized project, backing up any changed file under `.nullius/backups/` and never rewriting the user-owned seed files.
